...
_SCENARIOS = {
    ...
    "suspicious": {
        ...
        "sessions": 8,
        "auth_attempts": 5,
        "auth_failures": 1,       # 20% failure rate
        "pdu_attempts": 3,
        "pdu_failures": 0,
        "spike_count": 1,         # spikes in ~half the sessions
        "risk_flags": [{"flag_type": "auth_burst", "severity": "medium"}],
        "description": "Moderate auth failures + traffic spikes",
    },
    "anomalous": {
...
async def seed_scenarios():
    """Inject 3 simulated UEs (normal / suspicious / anomalous) into the
    live store so the summary and access-decision APIs return different
    classifications.  Safe to call multiple times — identities are
    deduplicated."""
    results = []
    ...
    for label, sc in _SCENARIOS.items():
        ...
        for i in range(sc["sessions"]):
            await db.insert_session(
                ...
                auth_attempts=sc["auth_attempts"],
                auth_failures=sc["auth_failures"],
                pdu_attempts=sc["pdu_attempts"],
                pdu_failures=sc["pdu_failures"],
                spike_count=sc["spike_count"],
